<?php
/**
 * Master 1-Click Fix & Permanent Phone Synchronizer
 * Upload to public_html/ and visit: https://www.rboaccounting.ae/update-phone.php
 */
...
if ( ! file_exists( __DIR__ . '/wp-load.php' ) ) {
	die( '<h2 style="font-family:sans-serif;color:red;">Error: Place update-phone.php directly in public_html/</h2>' );
}
...
// 1. Permanently Overwrite/Install the Updated MU-Plugin on the server
$mu_dir = WP_CONTENT_DIR . '/mu-plugins';
if ( ! is_dir( $mu_dir ) ) {
	@mkdir( $mu_dir, 0755, true );
}

$mu_file = $mu_dir . '/rbo-business-setup.php';
$new_plugin_code = <<<'PLUGIN_CODE_EOF'
<?php
/**
...
if ( @file_put_contents( $mu_file, $new_plugin_code ) ) {
	$log[] = "Successfully installed and locked updated <code>wp-content/mu-plugins/rbo-business-setup.php</code> with <strong>+971 50 805 1857</strong>";
} else {
	$log[] = "Warning: Could not write directly to <code>wp-content/mu-plugins/rbo-business-setup.php</code>. Please check file permissions.";
}

// 2. Also patch any other .php files in mu-plugins or root that have old numbers
$old_numbers = array(
	'+971 56 994 7840' => '+971 50 805 1857',
	'+971 50 682 9017' => '+971 50 805 1857',
	'+971 52 387 1857' => '+971 50 805 1857',
...
$files_to_scan = array_merge(
	(array) glob( $mu_dir . '/*.php' ),
	(array) glob( __DIR__ . '/*.php' )
);

foreach ( array_filter( $files_to_scan ) as $file ) {
	if ( basename( $file ) === 'update-phone.php' ) {
		continue;
	}
	$content = @file_get_contents( $file );
	if ( $content ) {
		$modified = false;
		foreach ( $old_numbers as $old => $new ) {
		...
		if ( $modified ) {
			...
			$log[] = 'Patched server file: <code>' . esc_html( basename( $file ) ) . '</code>';
		}
	}
}

// 3. Update Database (All tables: posts, postmeta, options)
foreach ( $old_numbers as $old => $new ) {
...
$log[] = "Updated all occurrences in database tables (posts, postmeta, options) to <strong>+971 50 805 1857</strong>";

// 4. Force run the newly embedded provisioning function to update all 6 pages in DB
if ( function_exists( 'rbo_get_business_setup_pages_spec' ) ) {
	$specs = rbo_get_business_setup_pages_spec();
	foreach ( $specs as $slug => $spec ) {
		$page = get_page_by_path( 'our-services/business-setup/' . $slug );
		if ( ! $page ) {
			$page = get_page_by_path( 'business-setup/' . $slug );
		}
		if ( ! $page ) {
	...
	$log[] = "Re-saved all 6 Business Setup pages with clean HTML and updated phone numbers";
}

// 5. Purge all caches
if ( function_exists( 'wp_cache_flush' ) ) {
	wp_cache_flush();
	$log[] = "Purged WordPress Object Cache";
}
if ( class_exists( '\Elementor\Plugin' ) ) {
	\Elementor\Plugin::$instance->files_manager->clear_cache();
	$log[] = "Purged Elementor Cache";
}
if ( defined( 'LSCWP_V' ) || class_exists( 'LiteSpeed\Purge' ) ) {
	do_action( 'litespeed_purge_all' );
	$log[] = "Purged LiteSpeed Cache";
}
if ( class_exists( 'SpeedyCache' ) && function_exists( 'speedycache_delete_cache' ) ) {
	speedycache_delete_cache();
	$log[] = "Purged SpeedyCache";
}
...
if ( is_dir( $cache_dir ) ) {
	...
	$log[] = "Deleted all server disk cache files";
}
...
<head>
<meta charset="utf-8">
<title>All Phone Numbers &amp; WhatsApp Fixed</title>
<style>
...
.num-banner { background: #0f1c30; border: 2px solid #22c55e; border-radius: 10px; padding: 18px; margin: 20px 0; text-align: center; }
.num-banner h2 { margin: 0 0 6px; font-size: 22px; color: #4ade80; }
.num-banner p { margin: 0; font-size: 14px; color: #94a3b8; }
.step { padding: 10px 14px; margin-bottom: 8px; background: #1e3152; border-radius: 8px; border-left: 4px solid #4ade80; font-size: 13px; color: #f1f5f9; }
.btn { display: inline-block; background: #22c55e; color: #ffffff !important; font-weight: 800; padding: 14px 28px; border-radius: 8px; text-decoration: none; margin-top: 20px; font-size: 15px; }
.btn:hover { background: #16a34a; }
</style>
</head>
<body>
<div class="card">
<h1>🎉 Live Website Permanently Fixed &amp; Synchronized!</h1>

<div class="num-banner">
	<h2>Active Number: +971 50 805 1857</h2>
	<p>WhatsApp Link: https://wa.me/971508051857</p>
</div>

<h3>Actions Executed:</h3>
<?php foreach ( $log as $item ) : ?>
	<div class="step">✓ <?php echo $item; ?></div>
<?php endforeach; ?>

<a href="<?php echo home_url( '/business-setup/?nocache=' . time() ); ?>" class="btn" target="_blank">Open Live Business Setup Page →</a>
</div>
</body>
</html>
